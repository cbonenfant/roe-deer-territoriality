...
No behavioural field observations were collected to confront movement
patterns with territoriality and rut. We therefore quantified the
predictive performance of movements patterns in two steps. We first
described at the population

by quantifying our ability to categorize individuals as males or
females from
...
\textsf{MASS} library implemented in R software, version 3.3.1 (R Core
Team, 2016). We used the second order Akaike’s information criterion
corrected for small sample size \citep[AICc,][]{burnham_model_1998}
and Akaike weights ($w$) to select the model with the most support
among the set of five candidate models constructed a priori. All
generalized additive mixed models were fitted using the \textsf{gamm}
function in the \textsf{mgcv} library \cite{wood_generalized_2006} in
\textbf{\textsf{R}} software.\nico{penses-tu qu'il soit nécessaire
